...
void downloadPatch(void*) {
    ...
    if (curl) {
		...
		snprintf(download_path, sizeof(download_path), "https://dl.awa.cool/scy/FPSLocker-Warehouse/SaltySD/plugins/FPSLocker/patches/%016lX/%016lX.yaml", TID, BID);
		...
		if (!temp_error_code) {
			...
			if (!temp_error_code) {
				...
				if (LOCK::tree.is_map(root_id) && LOCK::tree.find_child(root_id, "Addons") != c4::yml::NONE && !LOCK::tree["Addons"].is_keyval() && LOCK::tree["Addons"].num_children() > 0) {
					for (size_t i = 0; i < LOCK::tree["Addons"].num_children(); i++) {
						...
						std::string dpath = "https://dl.awa.cool/scy/FPSLocker-Warehouse/" + temp;
			...
			error_code = 0x404;
			curl_easy_setopt(curl, CURLOPT_URL, "https://dl.awa.cool/scy/FPSLocker-Warehouse/README.md");
